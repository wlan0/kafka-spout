...
/**
 * Storm spout reading messages from kafka, emitting them as single field
 * tuples.
 *
 * Implementation tracks a queue of message ids (partition and offset) and a set
 * of those ids that are pending to be acknowledged by the topology. The buffer
 * will only be populated with new message when *all* messages from the buffer
 * have been acknowledged because the {@link ConsumerConnector} allows
 * committing of the currently processed offset only through
 * {@link kafka.javaapi.consumer.ConsumerConnector#commitOffsets()}, which
 * commits *all* offsets that have been read, which does not necessarily
 * correspond to the offsets that were successfully processed by the storm
 * topology. Optimizing this behaviour is work for the (near) future.
 *
 * Aside from the properties used to configure the kafka consumer, the kafka
 * spout reads the following configuration parameters in storm configuration:
 * <ul>
 * <li>{@code kafka.spout.topic}: the kafka topic to read messages from (default
 * {@code storm});</li>
 * <li>{@code kafka.spout.fail.handler}: the policy to be used when messages
 * fail, whether to replay them, default {@code "reliable"} (either
 * {@code "reliable"}, {@code "unreliable"} or a fully qualified class name of
 * an implementation of {@link FailHandler});</li>
 * <li>{@code kafka.spout.consumer.group}: The kafka consumer group id.</li>
 * <li>{@code kafka.spout.buffer.size.max}: The maximum number of kafka messages
 * to buffer.</li>
 * </ul>
 *
 * @author Netherlands Forensics Institute
 */
public class KafkaSpoutSerial implements IRichSpout {
    private static final long serialVersionUID = -1L;
    private static final Logger LOG = LoggerFactory.getLogger(KafkaSpout.class);
    /**
     * Collection of messages being processed by the topology (either waiting to
     * be emitted or waiting to be acknowledged). Processed message offset is
     * committed when this is becomes empty.
     *
     * @see #fillBuffer()
     */
    ...
    /**
     * Creates a new kafka spout to be submitted in a storm topology.
     * Configuration is read from storm config when the spout is opened.
     */
    public KafkaSpoutSerial () {
    }

    /**
     * Convenience method assigning a {@link FailHandler} instance to this kafka
     * spout. If the configured value is {@code null},
     * {@link ConfigUtils#DEFAULT_FAIL_HANDLER} will be used, otherwise the
     * creation is delegated to
     * {@link ConfigUtils#createFailHandlerFromString(String)}.
     *
     * @param failHandler
     *            The configuration value for the failure policy.
     */
     ...
     *
     * @param config
     *            The storm configuration passed to
     *            {@link #open(Map, TopologyContext, SpoutOutputCollector)}.
     * @throws IllegalArgumentException
     *             When a required configuration parameter is missing or a
     *             sanity check fails.
     */
    ...
    /**
     * Refills the buffer with messages from the configured kafka topic if
     * available.
     *
     * @return Whether the buffer contains messages to be emitted after this
     *         call.
     * @throws IllegalStateException
     *             When current buffer is not empty or messages not acknowledged
     *             by topology.
     */
    protected boolean fillBuffer () {
        ...
        if (_iterator == null) {
            // create a stream of messages from _consumer using the streams as
            // defined on construction
            final Map<String, List<KafkaStream<byte[], byte[]>>> streams = _consumer.createMessageStreams(Collections
                .singletonMap(_topic, 1));
            _iterator = streams.get(_topic).get(0).iterator();
        }

        // We'll iterate the stream in a try-clause; kafka stream will poll its
        // client channel for the next message,
        // throwing a ConsumerTimeoutException when the configured timeout is
        // exceeded.
        try {
            int size = 0;
            ...
            // ignore, storm will call nextTuple again at some point in the near
            // future
            // timeout does *not* mean that no messages were read (state is
            // checked below)
        }
    ...
    public void declareOutputFields (final OutputFieldsDeclarer declarer) {
        // declare a single field tuple, containing the message as read from
        // kafka
    ...
    public void open (final Map config, final TopologyContext topology, final SpoutOutputCollector collector) {
        ...
        _topic = getTopic(config);

        _bufSize = getMaxBufSize(config);

        //read one message and ack it, before reading next
        overrideBufSizeTo(1);
    ...
    public void nextTuple () {
        // next tuple available when _queue contains ids or fillBuffer() is
        // allowed and indicates more messages were available
        // see class documentation for implementation note on the rationale
        ...
        if (!_queue.isEmpty() || (_inProgress.isEmpty() && fillBuffer())) {
            ...
            if (nextId != null) {
                ...
                // the next id from buffer should correspond to a message in the
                // pending map
                if (message == null) {
                    throw new IllegalStateException("no pending message for next id " + nextId);
                }
                // message should be considered a single object from Values'
                // point of view
                _collector.emit(new Values(message), nextId);
    ...
    public void fail (final Object o) {
        if (o instanceof KafkaMessageId) {
            ...
            if (_failHandler.shouldReplay(id)) {
                ...
                // remove message from pending; _failHandler will take action if
                // needed
    ...
    private void overrideBufSizeTo(Integer bufSize) {
        _bufSize = bufSize;
    }
}
